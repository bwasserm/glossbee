...
void Task3 ()
{
  ...
  // agr RF init used to be here, but was moved to other task to avoid startup race condition 
  ...
  while (1) {
	...
	// else nrk_kprintf( PSTR("rf tx success!\r\n" ));
      }
    }
    else {

      // Don't bother sending NCK
//      ack_buf[0] = 'N';
//      slip_tx (ack_buf, 1);
//    nrk_wait_until_next_period ();
...
static char uart1_rx_buf[GPS_BUF_LEN];//[MAX_RX_UART_BUF];
...
SIGNAL(USART1_RX_vect)
{
  ...
  if(uart1_rx_buf_end>=GPS_BUF_LEN) {
    	uart1_rx_buf_end=0; 
	}
  uart1_rx_signal=1;
  CLEAR_UART1_RX_INT();
